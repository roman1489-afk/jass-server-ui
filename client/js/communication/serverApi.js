	...
	handleMessageFromServer: (messageEvent) => {
		...
		switch (message.type) {
			...
			case MessageType.SUGGEST_TRUMPF.name:
				JassActions.suggestTrumpf(message.data);
				break;
			...
			case MessageType.SUGGEST_CARD.name:
				JassActions.suggestCard(message.data);
				break;
		...
		if (payload.source === 'VIEW_ACTION') {
			...
			switch (action.actionType) {
				...
				case JassAppConstants.AUTOJOIN_SESSION:
					action.data = {};
					action.data.isHuman = true;
					sendJSONMessageToClient(MessageType.CHOOSE_SESSION.name, SessionChoice.AUTOJOIN, action.data);
					break;
