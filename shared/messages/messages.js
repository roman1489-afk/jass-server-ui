...
function createSuggestTrumpf(gameType) {
	return {
		type: MessageType.SUGGEST_TRUMPF.name,
		data: gameType
	};
}
...
function createSuggestCard(card) {
	return {
		type: MessageType.SUGGEST_CARD.name,
		data: card
	};
}
...
function createChooseSession(sessionChoice, {sessionName, sessionType, asSpectator, advisedPlayerName, chosenTeamIndex, isHuman} = {}) {
	return {
...
export function create(messageType, ...data) {
	switch (messageType) {
		...
		case MessageType.SUGGEST_TRUMPF.name:
		...
		case MessageType.SUGGEST_CARD.name:
			return createSuggestCard(...data);
