...
describe('PlayerNames Component', () => {
	...
	it('should render a player div for each given player with the id where he sits, no class and an image', () => {
		...
		expect(playerNameElements[0].props.children[1].props.className).to.equal('hidden');
		expect(playerNameElements[0].props.children[1].props.children[1].props.children[1].props.className).to.equal('add-bot-icon');
		expect(playerNameElements[0].props.children[0].props.children).to.equal(props.players[0].name);
		...
		expect(playerNameElements[1].props.children[1].props.children[1].props.children[1].props.className).to.equal('add-bot-icon');
		expect(playerNameElements[1].props.children[0].props.children).to.equal(props.players[1].name);
	...
	it('should call joinBot onclick', () => {
		...
		shallowRenderer.render(React.createElement(PlayerNames, props));
		let actual = shallowRenderer.getRenderOutput();
		const playerRightOnClick = actual.props.children[1].props.children[1].props.children[1].props.onClick;
		expect(playerRightOnClick).to.be.a('function');
		try {
			playerRightOnClick();
		} catch (e) {
			// an error is thrown when the playerBoxId element's inner Text is reassigned. But we care about the starting of the bot here.
		}
